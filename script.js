...
// Load products for sale page
if (document.getElementById('sale-product-list')) {
    fetch('products.json')
        .then(response => response.json())
        .then(products => {
            const saleProducts = products.filter(product => product.onSale); // Assuming each product has an 'onSale' property
